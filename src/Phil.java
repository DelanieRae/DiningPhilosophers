public class Phil implements Runnable {
	...
	public void TrytoEat() {
		...
		if (myCup.acquireChopsticks()) {
			try {
				// System.out.println(Thread.currentThread().getName()+" eating all the
				// stuffs");
				Thread.sleep(timeToEat);
				eatCounter++;
				myCup.releaseChopsticks();
				think();
			} catch (InterruptedException e) {
				System.err.println("Tried to grab the chopsticks and something went wrong");
			}
		} else {
			// System.out.println("Could not get chopsticks");
			think();
		}
	}
